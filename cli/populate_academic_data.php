...
if ($options['clear']) {
    ...
    $DB->delete_records('local_att_rooms');
    $DB->delete_records('local_att_slots');
    cli_writeln("Cleaned plugin tables: local_att_schedules, local_att_rooms, local_att_slots.");
}
...
// ---------------------------------------------------------
cli_writeln("\n[4/5] Populating Campus Rooms (local_att_rooms)...");
...
for ($r = 0; $r < $numrooms; $r++) {
    $roomdata = $allrooms[$r];
    $existing = $DB->get_record('local_att_rooms', ['name' => $roomdata['name']]);
    if (!$existing) {
        $DB->insert_record('local_att_rooms', (object)$roomdata);
        $addedrooms++;
    }
}
...
// ---------------------------------------------------------
cli_writeln("\n[5/5] Populating Master Time Slots (local_att_slots)...");
...
for ($s = 0; $s < $numslots; $s++) {
    $slotdata = $allslots[$s];
    $existing = $DB->get_record('local_att_slots', [
        'type'      => $slotdata['type'],
    ...
    if (!$existing) {
        $DB->insert_record('local_att_slots', (object)$slotdata);
    }
}
...
cli_writeln("  - Total Campus Rooms: " . $DB->count_records('local_att_rooms'));
cli_writeln("  - Total Time Slots:   " . $DB->count_records('local_att_slots'));
